# OBS! Denna fil är ännu inte färdig. Läs på egen risk. 
...
Anteckningarna i alla kodfiler är gjord på engelska. Vid rättningen har jag inte haft någon aning om vems kod som jag har rättat, även om jag naturligtvis kunnat ta reda på det om jag ville. I enstaka fall har dessutom studenter skrivit sina namn i koden. 
...
Med detta fel avses antingen att samma minnesutrymme har frigjorts fler än en gång, eller att `free` anropats på en pekare till något som inte allokerats med `malloc`. 

Exempel på program som gjort sig skyldiga till **invalid free** är program som försöker frigöra elementen i köerna. Dessa har ju typen `void *` och är därför inte vettiga att anropa `free` med eftersom de skulle kunna vara en komplicerad länkad struktur som måste tas bort av en specifik konstruktor. 
...
Med detta fel avses att programmet frigör minne innan det är klart med det. Ett "chockerande" exempel är flera förekomster av program som gör t.ex. 
...
På raden `!!!` ovan tar man alltså bort den nod som man just skapat. Det är oklart varför. Kanske tror man att `node_t *` har värdesemantik och har kopierats in i kön? Ett litet program som först allokerar, och sedan frigör utan att allokera igen kan råka fungera trots att man läser frigjort minne. Kanske kan man stoppat in ett anrop till `free` i syfte att få bort felmeddelanden om minnesläckage i valgrind och ignorerat att man bytt läckagen mot invalid reads? 
...
Ett fel som ett litet antal studenter har gjort är att göra allokeringar i `dequeue`, alltså där minne skall frigöras. Det rör sig förmodligen inte om att man räkat vända på `enqueue` och `dequeue` -- variabelnamn varslar om förståelse. Ofta är det t.ex. kod som ser ut så här:
...
## Error 6: invalid read

Invalid read är ett felmeddelande i valgrind som orsakas av läsningar från minne som inte har initierats. Följande skulle ge upphov till en invalid read:

```c
node_t *n = malloc(sizeof(node_t));
void *elem = n->elem; /// Invalid read!
```
...
Det var elva som skrev Java-delen. Dessa har fått feedback direkt
i programmet.
...
Av de som blev underkända hade 3 inte implementerat en länkad
lista *själva*, som var uppgiften, utan istället använt Javas egna
listor. Ett dumt fel hade smugit sig in i uppgiftstexten och
förvirrade någon enstaka -- ingen har fått rest eller blivit
underkänd på grund av detta. 
